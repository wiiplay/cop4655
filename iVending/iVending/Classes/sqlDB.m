...
- (BOOL) buildDatabaseStructure
{
    ...
    if (sqlite3_open(dbpath, &(vendingDB)) == SQLITE_OK) {
        ...
        const char *sql_stmt = "PRAGMA foreign_keys = off; CREATE TABLE vendingContent ( contentID  INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, machine_ID INTEGER NOT NULL REFERENCES machines (machineID), product_ID INTEGER REFERENCES products (ID) NOT NULL, itemRow INTEGER NOT NULL, itemColumn INTEGER NOT NULL,  quantity INTEGER NOT NULL, cost DECIMAL NOT NULL ); PRAGMA foreign_keys = on;";
